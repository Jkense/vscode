...
export class NativePolicyService extends AbstractPolicyService implements IPolicyService {
	...
	protected async _updatePolicyDefinitions(policyDefinitions: IStringDictionary<PolicyDefinition>): Promise<void> {
		...
		try {
			createWatcher = (await import('@vscode/policy-watcher')).createWatcher;
		} catch (err) {
			this.logService.error(`NativePolicyService#_updatePolicyDefinitions - Failed to load @vscode/policy-watcher native module, policy watching is disabled:`, err);
			return;
		}
